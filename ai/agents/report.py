...
def generate_report_deterministic(
    ...
    for f in all_findings:
        mitre = (
            f.mitre_techniques if hasattr(f, "mitre_techniques") else f.get("mitre_techniques", [])
        )
    ...
    sections = [
        ReportSection(
            ...
            content=exec_summary.overview,
            order=1,
        ),
        ReportSection(
            section_id="risk_score",
            title="Risk Score & Classification",
            content=f"Score: {score}/100, Tier: {tier}, Category: {category}",
            order=2,
        ),
        ReportSection(
            section_id="technical",
            title="Technical Analysis",
            content="See technical_details",
            order=3,
        ),
        ReportSection(
            section_id="iocs",
            title="Indicators of Compromise",
            content="See evidence_catalog",
            order=4,
        ),
        ReportSection(
            section_id="mitre",
            title="MITRE ATT&CK Mapping",
            content=f"Techniques: {', '.join(sorted(mitre_techniques))}",
            order=5,
        ),
        ReportSection(
            section_id="recommendations",
            title="Recommendations",
            content="\n".join(exec_summary.recommended_actions),
            order=6,
        ),
    ]
