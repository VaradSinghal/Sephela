...
from ai.schemas.threat_intel import (
    IOCMatch,
    MalwareFamily,
    ThreatIntelAnalysis,
    TIConnection,
...
MALWARE_FAMILIES_BANKING = {
    "Anubis": {
        "mitre": ["T1417", "T1636.004", "T1582"],
        "overlay": True,
        "accessibility": True,
        "sms": True,
    },
    "Cerberus": {
        "mitre": ["T1417", "T1636.004"],
        "overlay": True,
        "accessibility": True,
        "sms": True,
    },
...
def analyze_threat_intel_deterministic(
    ...
    return ThreatIntelAnalysis(
        ...
        malicious_hash_matches=len(
            [m for m in hash_matches if m.severity in (Severity.high, Severity.critical)]
        ),
        malicious_domain_matches=len(
            [m for m in domain_matches if m.severity in (Severity.high, Severity.critical)]
        ),
        malicious_ip_matches=len(
            [m for m in ip_matches if m.severity in (Severity.high, Severity.critical)]
        ),
