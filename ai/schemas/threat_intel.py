...
class IOCMatch(BaseModel):
    ...
    indicator_type: str = Field(
        ..., pattern="^(hash|domain|ip|url|mutex|registry|file_path|certificate)$"
    )
    source: str
    confidence: Confidence
    severity: Severity
...
class TIConnection(Finding):
    ...
    connection_type: str = Field(
        ...,
        pattern="^(ioc_match|family_attribution|actor_attribution|campaign_link|infrastructure_overlap)$",
    )
...
class ThreatIntelAnalysis(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        self.total_ioc_matches = (
            len(self.hash_matches)
            + len(self.domain_matches)
            + len(self.ip_matches)
            + len(self.url_matches)
            + len(self.cert_matches)
        )
        self.malicious_hash_matches = sum(
            1 for m in self.hash_matches if m.severity in (Severity.high, Severity.critical)
        )
        self.malicious_domain_matches = sum(
            1 for m in self.domain_matches if m.severity in (Severity.high, Severity.critical)
        )
        self.malicious_ip_matches = sum(
            1 for m in self.ip_matches if m.severity in (Severity.high, Severity.critical)
        )
